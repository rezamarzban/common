...
• HTTP server (port 8000) – serves a page with the latest coordinate + speed
• log.txt   – console output (thread‑safe) + raw packet dumps + speed
• log.html  – one line per GPS URI + speed (thread‑safe)
"""
...
latest_lon = None
latest_speed = None        # km/h (int)
state_lock = threading.Lock()    # protects latest_lat/lon/speed
...
def append_uri(uri: str, speed: int) -> None:
    """Append a GPS URI + speed to log.html (thread‑safe)."""
    ts = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    link = f'<p><a href="{uri}">{ts} Open map</a></p>'
    speed_line = f'<p>Speed: {speed} km/h</p>'
    with log_lock:
                        ...
                        if proto == 34:
                            try:
                                ...
                                with state_lock:
                                    ...
                                    latest_lon = lon
                                    latest_speed = speed
                                log(f"[GPS] lat={lat:.6f}, lon={lon:.6f}, speed={speed} km/h")

                                uri = f"geo:{lat:.6f},{lon:.6f};u=35"
                                append_uri(uri, speed)
...
# ------------------------------------------------------------------
# HTTP SERVER – simple page with the latest GPS coordinate + speed
# ------------------------------------------------------------------
