...
def encrypt_lite(data, password):
    b64_encoded = base64.b64encode(data.encode()).hex()
    data_int = int(b64_encoded, 16)
    pwd_int = int.from_bytes(password.encode(), 'big')
    encrypted_int = data_int * pwd_int
    encrypted_hex = encrypted_int.to_bytes((encrypted_int.bit_length() + 7) // 8, 'big').hex()
    length_prefix = len(b64_encoded).to_bytes(4, 'big').hex()
    return f"{length_prefix}:{encrypted_hex}"

def decrypt_lite(encrypted_data, password):
    length_hex, encrypted_hex = encrypted_data.split(':')
    original_len = int.from_bytes(bytes.fromhex(length_hex), 'big')
    encrypted_int = int.from_bytes(bytes.fromhex(encrypted_hex), 'big')
    pwd_int = int.from_bytes(password.encode(), 'big')
    decrypted_int = encrypted_int // pwd_int
    b64_bytes = decrypted_int.to_bytes(original_len, 'big')
    return base64.b64decode(b64_bytes).decode()

def log(msg: str) -> None:
    ts = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(f"{ts}  {encrypt_lite(msg, password)}", file=log_fp, flush=True)
    print(f"{ts}  {msg}")

def append_uri(uri: str, speed: int, dt_str: str) -> None:
    ts = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(f'<p><a href="{encrypt_lite(uri, password)}">{dt_str} Open map</a></p>', file=html_fp, flush=True)
    print(f"<p>Speed: {speed} km/h</p>", file=html_fp, flush=True)
    print(f"<p>{ts}</p>", file=html_fp, flush=True)
